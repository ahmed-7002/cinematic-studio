  ...
  return (
    ...
    >
      <div className="aspect-[2/3] overflow-hidden relative">
        ...
        <div className="absolute bottom-1 left-1 right-1 sm:bottom-2 sm:left-2 sm:right-2 transform translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-200 transform-gpu will-change-transform">
          <div className="flex items-center justify-between text-white text-xs sm:text-sm">
            ...
            <span className="bg-black/50 px-1 py-0.5 sm:px-2 sm:py-1 rounded-full text-xs">
      ...
      <div className="p-3 sm:p-4">
        <h3 className="font-bold text-sm sm:text-base lg:text-lg text-gray-900 dark:text-white mb-1 sm:mb-2 line-clamp-2 leading-tight">
          {title}
        </h3>
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-lg rounded-3xl max-w-5xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-y-auto shadow-2xl border border-white/20 dark:border-gray-700/50">
        <div className="relative">
          ...
          <button
            onClick={onClose}
            className="absolute top-2 right-2 sm:top-4 sm:right-4 p-2 sm:p-3 bg-black/50 backdrop-blur-sm rounded-full text-white hover:bg-black/70 transition-all duration-300 hover:scale-110"
          >
          ...
          {trailer && (
            <button
              ...
              className="absolute bottom-2 left-2 sm:bottom-4 sm:left-4 flex items-center space-x-1 sm:space-x-2 bg-red-600 hover:bg-red-700 text-white px-3 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-xl transition-all duration-300 hover:scale-105 shadow-lg"
            >
              <Play className="w-3 h-3 sm:w-5 sm:h-5" />
              <span className="font-semibold text-xs sm:text-base">Watch Trailer</span>
            </button>
          )}

          {/* Content type badge */}
          <div className="absolute top-2 left-2 sm:top-4 sm:left-4">
            <div className={`flex items-center space-x-1 sm:space-x-2 px-2 py-1 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl backdrop-blur-sm ${
              contentType === CONTENT_TYPES.MOVIE 
                ? 'bg-blue-600/90 text-white' 
                : 'bg-purple-600/90 text-white'
            }`}>
              {contentType === CONTENT_TYPES.MOVIE ? 
                <Film className="w-3 h-3 sm:w-5 sm:h-5" /> : 
                <Tv className="w-3 h-3 sm:w-5 sm:h-5" />
              }
            ...
            <>
              <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 lg:gap-8">
                ...
                <div className="lg:w-2/3">
                  <h1 className="text-xl sm:text-2xl lg:text-3xl xl:text-4xl font-bold text-gray-900 dark:text-white mb-4 sm:mb-6 leading-tight">
                    {title}
                  </h1>
                  
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 lg:gap-4 mb-4 sm:mb-6">
                    <div className="flex items-center bg-yellow-100 dark:bg-yellow-900/30 px-2 py-1 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl">
                      <Star className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 fill-yellow-500 text-yellow-500 mr-1 sm:mr-2" />
                      <span className="font-bold text-gray-900 dark:text-white text-sm sm:text-base lg:text-lg">
                        {content.vote_average?.toFixed(1)}/10
                      </span>
                      <span className="ml-1 sm:ml-2 text-gray-600 dark:text-gray-400 text-xs sm:text-sm">
                        ({content.vote_count?.toLocaleString()} votes)
                      </span>
                    </div>
                    
                    <div className="flex items-center text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800/50 px-2 py-1 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl">
                    ...
                    {contentType === CONTENT_TYPES.MOVIE && contentDetails?.runtime && (
                      <div className="flex items-center text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800/50 px-2 py-1 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl">
                        ...
                        <span className="font-medium text-xs sm:text-sm">{contentDetails.runtime} min</span>
                      </div>
                    )}
                    ...
                    <div className="flex items-center text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800/50 px-2 py-1 sm:px-3 sm:py-2 rounded-lg sm:rounded-xl">
                      ...
                      <span className="font-medium text-xs sm:text-sm">{content.original_language?.toUpperCase()}</span>
                    </div>
                  </div>

                  {contentDetails?.genres && (
                    <div className="mb-4 sm:mb-6">
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mb-2 sm:mb-3">Genres</h3>
                      <div className="flex flex-wrap gap-1 sm:gap-2">
                        {contentDetails.genres.map(genre => (
                          <span 
                            key={genre.id}
                            className="px-2 py-1 sm:px-3 sm:py-1 lg:px-4 lg:py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg sm:rounded-xl text-xs sm:text-sm font-medium shadow-lg hover:shadow-xl transition-shadow duration-300"
                          >
                            {genre.name}
                          </span>
                        ))}
                  ...
                  {contentType === CONTENT_TYPES.MOVIE && director && (
                    <div className="mb-4 sm:mb-6">
                      ...
                      <p className="text-gray-700 dark:text-gray-300 text-sm sm:text-base lg:text-lg font-medium">{director.name}</p>
                    </div>
                  )}
                  ...
                  {contentType === CONTENT_TYPES.TV && creator && (
                    <div className="mb-4 sm:mb-6">
                      ...
                      <p className="text-gray-700 dark:text-gray-300 text-sm sm:text-base lg:text-lg font-medium">{creator.name}</p>
                    </div>
                  )}
                    ...
                    <div className="mb-4 sm:mb-6">
                      ...
                      <div className="flex flex-wrap gap-1 sm:gap-2">
                        {contentDetails.networks.map(network => (
                          <span 
                            key={network.id}
                            className="px-2 py-1 sm:px-3 sm:py-1 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg text-xs sm:text-sm font-medium"
                          >
                            {network.name}
                          </span>
                        ))}
                  ...
                  {mainCast.length > 0 && (
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                        {mainCast.map(actor => (
                          <div key={actor.id} className="bg-gray-50 dark:bg-gray-800/50 p-3 sm:p-4 rounded-lg sm:rounded-xl">
...
const FilterSection = ({ genres, selectedGenre, onGenreChange, selectedYear, onYearChange, selectedLanguage, onLanguageChange, contentType }) => {
  ...
  return (
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-2xl sm:rounded-3xl shadow-xl p-3 sm:p-4 lg:p-6 mb-6 sm:mb-8 border border-white/20 dark:border-gray-700/50">
      <div className="flex items-center mb-4 sm:mb-6">
        <Filter className="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3 text-blue-600 dark:text-blue-400" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
        <div>
          ...
          <select
            ...
            className="w-full p-2 sm:p-3 lg:p-4 border-2 border-gray-200 dark:border-gray-600 rounded-lg sm:rounded-xl bg-white/90 dark:bg-gray-700/90 text-gray-900 dark:text-white focus:ring-4 focus:ring-blue-500/50 focus:border-blue-500 transition-all duration-300 font-medium text-sm sm:text-base"
          >
...
const App = () => {
  ...
  const fetchContent = async () => {
    ...
    try {
      const currentDate = new Date().toISOString().split('T')[0]; // Get current date in YYYY-MM-DD format (2025-07-27)
      ...
      // Set up date filtering based on whether a specific year is selected
      if (selectedYear) {
        // When a specific year is selected, only show content from that year
        if (contentType === CONTENT_TYPES.MOVIE) {
          params.year = selectedYear;
          // For current year (2025), limit to current date
        ...
        // No specific year selected, show all content up to current date
        if (contentType === CONTENT_TYPES.MOVIE) {
          params['release_date.lte'] = currentDate;
      ...
      if (debouncedSearchQuery.trim()) {
        ...
        // Remove sort_by for search endpoint
        delete params.sort_by;
        // Keep the date filters for search results too
      }
      ...
      // Filter out any upcoming releases that might still appear and sort results by date in descending order
      let filteredResults = (data.results || []).filter(item => {
        ...
        // Only include content released up to the current date (July 27, 2025)
        return itemDate <= currentDateObj;
      });

      // Sort by date in descending order (newest first, from July 27, 2025 backwards)
